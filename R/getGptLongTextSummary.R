...
#' (1) splits long text into chunks that fit within token limits and summarizes each chunk,
#' (2) combines chunk summaries into a final consolidated summary. Handles token estimation
#' and automatic chunking to prevent API limits from being exceeded.
#'
...
#' @param gpt.base.url Character string for the GPT API base URL (default: "https://api.openai.com")
#' @param gpt.model Character string specifying the GPT model to use (default: "gpt-4o-mini")
#' @param system.prompt.stage1 Character string containing the system prompt for chunk summarization
#' @param user.prompt.stage1 Character string containing the user prompt template for chunk summarization (default: "Please summarize the following events:\n\n{text}")
#' @param system.prompt.stage2 Character string containing the system prompt for final consolidation
#' @param period.label Character string providing temporal context for the summary (optional, default: NULL)
#' @param max.input.tokens Numeric value for maximum input tokens per API call (default: 120000)
#' @param gpt.temperature Numeric value for GPT temperature parameter (default: 0)
#' @param gpt.max.output.tokens Numeric value for maximum output tokens from GPT (default: 2048)
#' @param token.multiplier Numeric multiplier for converting word count to estimated tokens (default: 1.3)
...
#'   user.prompt.stage1 = "Identify main actors and event types in:\n\n{text}",
#'   system.prompt.stage2 = "Synthesize these analyses into coherent overview.",
#'   user.prompt.stage2 = "Create final summary from:\n\n{text}",
...
getGptLongTextSummary <- function(text.vec,
  ...
  if (missing(text.vec) || !is.character(text.vec) || length(text.vec) == 0) {
    stop("text.vec must be a non-empty character vector")
  }

  if (missing(gpt.token) || !is.character(gpt.token) || nchar(gpt.token) == 0) {
    stop("gpt.token must be a valid non-empty character string")
  }

  if (missing(system.prompt.stage1) || !is.character(system.prompt.stage1)) {
    stop("system.prompt.stage1 must be provided as a character string")
  }

  if (missing(system.prompt.stage2) || !is.character(system.prompt.stage2)) {
    stop("system.prompt.stage2 must be provided as a character string")
  }
  ...
  tryCatch({
    ...
    entry.token.counts <- purrr::map_dbl(text.vec, function(entry) {
      word.count <- length(tokenizers::tokenize_words(entry)[[1]])
      return(ceiling(word.count * token.multiplier))
    })

    # Calculate prompt overhead for stage 1
    stage1.user.prompt.base <- gsub("\\{text\\}", "", user.prompt.stage1)
    ...
    for (i in seq_along(text.vec)) {
      ...
      if (current.chunk.tokens + entry.tokens > effective.max.tokens && length(current.chunk) > 0) {
    ...
    chunk.summaries.vec <- purrr::map_chr(seq_along(chunks.ls), function(chunk.idx) {

      chunk.data <- chunks.ls[[chunk.idx]]
      chunk.text <- paste(chunk.data$text, collapse = "\n\n")

      message("    - Processing chunk ", chunk.idx, "/", length(chunks.ls),
              " (", chunk.data$entry.count, " entries, ~", chunk.data$token.count, " tokens)")

      # Create messages for this chunk
      user.prompt.filled <- glue::glue(user.prompt.stage1,
                                       text = chunk.text,
                                       period_context = period.context)
      messages <- jrcgpt::startGptChat(system.prompt.stage1, role = "system")
      messages <- jrcgpt::addGptMessage(messages, user.prompt.filled, role = "user")
    ...
    if (length(chunk.summaries.vec) == 1) {
      ...
      user.prompt.stage2.filled <- glue::glue(user.prompt.stage2,
                                              text = combined.summaries,
                                              period_context = period.context)
      messages.stage2 <- jrcgpt::startGptChat(system.prompt.stage2, role = "system")
      messages.stage2 <- jrcgpt::addGptMessage(messages.stage2, user.prompt.stage2.filled, role = "user")
    ...
    # Create chunk details data frame
    chunk.details.df <- data.frame(
      chunk.id = seq_along(chunks.ls),
      entry.count = sapply(chunks.ls, function(x) x$entry.count),
      estimated.tokens = sapply(chunks.ls, function(x) x$token.count),
    ...
    message("  * Final summary length: ", nchar(result.ls$final.summary), " characters")
